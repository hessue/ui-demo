...
namespace BlockAndDagger.MenuControls
{
    public sealed class MenuControls : MonoBehaviour
    {
        ...
        private ActiveLevelPanLimiter _panLimiter;
        ...
        public void SetPreviewPanLimiter(Level level, float paddingMeters = 0f, float extraBottomPaddingMeters = 0f, float extraTopPaddingMeters = 0f)
        {
            if (_panLimiter == null)
            {
                _panLimiter = new ActiveLevelPanLimiter(level, paddingMeters, extraBottomPaddingMeters, extraTopPaddingMeters);
                return;
            }

            _panLimiter.SetPanLimiter(level, paddingMeters, extraBottomPaddingMeters, extraTopPaddingMeters);
        }
        ...
        private void Update()
        {
            ...
            if (m_detectionEnabled)
            {
                ...
                _lastPointerDelta = _uiSwipeDetector?.LastPointerDelta ?? Vector2.zero;
                UpdateMovement();
            }
        }
        ...
        private void UpdateMovement()
        {
            ...
            var desiredPosition = _cam.transform.position + _smoothedVelocity * dt;
            if (_panLimiter != null && _panLimiter.IsEnabled && _panLimiter.IsActive(_cam))
            {
                desiredPosition = _panLimiter.ClampPosition(desiredPosition, _cam);
            }
