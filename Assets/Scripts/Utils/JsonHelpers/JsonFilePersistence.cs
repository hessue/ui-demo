...
using UnityEngine;
using BlockAndDagger.Utils.JsonHelpers;
...
namespace BlockAndDagger
{
    ...
    public class JsonLevelData
    {
        ...
        public ManifestEntry[] addressableAssets;
    ...
    {
        public string guid; //not good idea to rely upon if there are any changes made later
    ...
    public class JsonFilePersistence
    {
        ...
        public void Save(LevelData data, bool useEditedVersion)
        {
            ...
            var jo = JObject.FromObject(conversion, JsonSerializer.Create(new JsonSerializerSettings { NullValueHandling = NullValueHandling.Ignore }));
            var manifestProp = jo.Property("addressableAssets");
            if (manifestProp != null)
            {
                manifestProp.Remove();
                jo.AddFirst(manifestProp);
            }
            var json = jo.ToString(Newtonsoft.Json.Formatting.Indented);
        ...
        public string ReadFromPersistentDataPath(string fileName)
        {
            ...
            if (File.Exists(path))
            {
                using (StreamReader reader = new StreamReader(path))
                {
                    ...
                    // Backwards compatibility: older files used 'manifestEntries' key
                    if (json.Contains("\"manifestEntries\""))
                    {
                        json = json.Replace("\"manifestEntries\"", "\"addressableAssets\"");
                    }
                    return json;
                }
            }
        ...
        public string ReadFromReadOnlyFolder(string fileName, string subfolder = "")
        {
            ...
            if (!string.IsNullOrEmpty(json) && json.Contains("\"manifestEntries\""))
            {
                json = json.Replace("\"manifestEntries\"", "\"addressableAssets\"");
            }
