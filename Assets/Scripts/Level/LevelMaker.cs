...
namespace BlockAndDagger
{
    public sealed class LevelMaker : MonoBehaviour
    {
        ...
        public async Task LoadLevelToLevelMaker(LevelAndBlueprint levelAndBlueprint)
        {
            ...
            var usedGroups = GetUsedTileTypeGroupsDistinct(levelAndBlueprint);
            await AddressablesManager.ReleasePreloadedGroupAsync(usedGroups);
           
            
            //await AddressablesValidator.ValidateGroupsExist(groups);

#if UNITY_EDITOR
            if (usedGroups == null)
            {
                //Fallback for devs to generate levels again in case of issues
                await AddressablesManager.StartPreloadGroupAssets(new[] { "dev_blocks_all" });
                Debug.LogWarning("Loaded blocks from all blocks addressable. ONLY FOR DEV");
            }
#endif
            if(usedGroups != null)
            {
                await AddressablesManager.StartPreloadGroupAssets(usedGroups);
                Debug.Log($"Preloaded {String.Join(", ", usedGroups)} addressable groups for level {levelAndBlueprint.Level}");
            }
        ...
        private List<string> GetUsedTileTypeGroupsDistinct(LevelAndBlueprint levelAndBlueprint)
        {
            JsonLevelData json = null;
            json = _dataPersistenceManager.LoadEmptyLevel(levelAndBlueprint.Level);
            if (json == null)
            {
                return null;
            }

            if (json.addressableAssets == null || json.addressableAssets.Length == 0)
            {
                return null;
            }

            var result = json.addressableAssets.Select(x => x.group).Distinct().ToList();
            return result.Select(x => x.ToLower()).ToList();
        }
        ...
        private void AddToEmptyPosition(IFocusableBlock player, TileType blockType)
        {
            ...
            else if (_levelBlockPointing != null && _levelBlockPointing.GetCachedPreviewBlockRotation().HasValue)
            {
                rotation = _levelBlockPointing.GetCachedPreviewBlockRotation().Value;
            }
        ...
        private Quaternion DetermineReplaceRotation(IFocusableBlock blockInfo)
        {
            ...
            {
                replaceRotation = _levelBlockPointing.GetCachedPreviewBlockRotation().Value;
            }
        ...
        public void RotateCurrentlyFocusedBlock(IFocusableBlock player)
        {
            ...
            if (focusedTransform != null)
            {
                ...
                else
                {
                    ...
                    {
                        _levelBlockPointing.CachePreviewBlockRotation(focusedTransform.rotation);
                    }
                }
            ...
            if (ghostTransform != null)
            {
                ...
                else
                {
                    ...
                    if (_levelBlockPointing != null)
                    {
                        _levelBlockPointing.CachePreviewBlockRotation(ghostTransform.rotation);
                    }
                }
            ...
            if (ghostBlockRot != null)
            {
                ...
                if (ghostGo != null)
                {
                    ...
                    else
                    {
                        ...
                        if (_levelBlockPointing != null)
                        {
                            _levelBlockPointing.CachePreviewBlockRotation(newRot);
                        }
                    }
                    ...
                    if (_levelBlockPointing != null)
                    {
                        _levelBlockPointing.CachePreviewBlockRotation(ghostGo.transform.rotation);
                    }
        ...
        IEnumerator RotateSlowly(Transform target, float angle, Transform ghostBlock = null)
        {
            ...
            if (_levelBlockPointing != null)
            {
                _levelBlockPointing.CachePreviewBlockRotation(target.rotation);
            }
