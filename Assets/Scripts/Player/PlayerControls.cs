...
namespace BlockAndDagger
{
    ...
    public sealed partial class PlayerControls : MonoBehaviour
    {
        ...
        private IMenuFacade _gm;
        private IngameUI _ingameUI;
        private IDevCheatService _devCheatService;

        [Inject]
        public void Construct(IMobileAudioManager audioManager, IGameManager gameManager)
        {
            _m_audioManager = audioManager;
            _gm = gameManager;
            _ingameUI =  gameManager.IngameUI;
            _devCheatService = gameManager;
        }

        public void ManualConstruct(IMobileAudioManager audioManager, IGameManager gameManager)
        {
            _m_audioManager = audioManager;
            _gm = gameManager;
            _ingameUI = gameManager.IngameUI;
        ...
        public void ToggleIngameMenu(InputAction.CallbackContext ctx)
        {
            if (ctx.performed)
            {
                ...
                _ingameUI.ToggleMenuButtonClicked();
            }
        }
        ...
        private void DevCheatsCompleteLevel(InputAction.CallbackContext ctx)
        {
            if (ctx.performed)
            {
                _m_audioManager.StopMusic();
                 _gm.ProceedToNextLevel();
            }
        }
        ...
        private void DevCheatsKillPlayer(InputAction.CallbackContext ctx)
        {
            ...
            {
                _m_audioManager.StopMusic();
                _devCheatService.Players[0].Kill("Cheats Death");
            }
        }
#endif
